...
def flight_status(flight_id,year,month,date):
    ...
    dep_airport = soul.find_all("div",class_='text-helper__TextHelper-sc-8bko4a-0 cHdMkI')[0].text
    ...
    return 
    status,arr_city,arr_airport,dep_city,dep_airport,scheduled_arr_time,actual_arr_time,scheduled_dep_time,actual_dep_time,terminal_arr,gate_arr,terminal_dep,gate_dep

def main():
        print(Fore.GREEN+"--"*40+Fore.RESET)
        print(Fore.CYAN+"""Tool By: ohokaynice ::))"""+Fore.RESET)
        while True:
            print(Fore.GREEN+"--"*40+Fore.RESET)
            print(Fore.GREEN+"""
            ...
            if choice==1:
                ...
                arr_data = get_airport_iata(arrival_airport_country)
                if len(arr_data)==0:
                ...
                dep_data = get_airport_iata(departure_airport_country)
